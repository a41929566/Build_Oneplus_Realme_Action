...
if [ "$ONCE_MODE" = "false" ]; then
    echo "$$" > "$PID_FILE"
    trap 'rm -f "$PID_FILE"; if [ -w /sys/module/pkgmask/parameters/hide_proc_names ]; then echo "" > /sys/module/pkgmask/parameters/hide_proc_names 2>/dev/null; echo 1 > /sys/module/pkgmask/parameters/reload 2>/dev/null; fi' EXIT
fi
...
handle_action() {
    ...
    case "$a" in
        ...
        save_hide:*)
            ...
            local listA="" listB=""
            # 用 IFS='|' read 拆第一个竖线；read 会把剩余部分全给 listB
            local oldifs="$IFS"
            IFS='|' read -r listA listB 2>/dev/null <<EOF
$payload
EOF
            IFS="$oldifs"
            [ -z "$listA" ] && listA=""
            [ -z "$listB" ] && listB=""
            log 2 "save_hide split: A=[$listA] B=[$listB]"
            local A_space B_space
            A_space=$(echo "$listA" | tr ',' ' ')
            B_space=$(echo "$listB" | tr ',' ' ')
            ...
            ;;
        det_add:*)
            cfg_add pkgmask_targets "${a#det_add:}"
            sh "$MODDIR/tools/pkgmask_setup.sh" apply
            ;;
        det_del:*)
            cfg_del pkgmask_targets "${a#det_del:}"
            sh "$MODDIR/tools/pkgmask_setup.sh" apply
            ;;
        hide_pkg_add:*)
            cfg_add pkgmask_hide_pkgs "${a#hide_pkg_add:}"
            sh "$MODDIR/tools/pkgmask_setup.sh" apply
            ;;
        hide_pkg_del:*)
            cfg_del pkgmask_hide_pkgs "${a#hide_pkg_del:}"
            sh "$MODDIR/tools/pkgmask_setup.sh" apply
            ;;
...
# v5: 随机 15 hex comm + '!' 前缀进入 stealth 名单（对任何读者含 root 隐藏）
SC=$(head -c 8 /dev/urandom 2>/dev/null | od -An -tx1 2>/dev/null | tr -d ' \n' | cut -c1-15)
[ -z "$SC" ] && SC="d$$"
echo "$SC" > "/proc/$$/comm" 2>/dev/null
...
if [ -w /sys/module/pkgmask/parameters/hide_proc_names ]; then
    ...
    # 清掉可能残留的旧 kworker/u16:99 或上次的随机串
    _new=""
    for _item in $(echo "$_cur" | tr ',' ' '); do
        case "$_item" in
            kworker/u16:99|[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]) continue ;;
            "") continue ;;
        esac
        _new="${_new:+$_new,}$_item"
    done
    echo "${_new:+$_new,}!$SC" > "$_hpn" 2>/dev/null
