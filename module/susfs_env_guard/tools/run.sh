...
if [ -f "$MODDIR/tools/daemon_loop.sh" ]; then
    ...
    # v5: setsid 起新会话，避免 nohup 不建 session 导致 daemon.pid 残留死进程
    (setsid sh "$MODDIR/tools/daemon_loop.sh" >> "$RUN_DIR/daemon.boot.log" 2>&1 &) 2>/dev/null || \
        nohup sh "$MODDIR/tools/daemon_loop.sh" >> "$RUN_DIR/daemon.boot.log" 2>&1 &
    log_file "daemon_loop.sh started (setsid/nohup)"
    sleep 3
fi
