...
static bool binder_enabled;   /* v4.6: 默认关闭；binder.c 注入默认也不编译，双保险 */
 ...
 *
 * Single exported entry point for binder.c: performs UID gating and package
 * scrubbing in one call, so only ONE pkgmask symbol is visible to kallsyms.
 */
int pmk_filter_binder(char *data, size_t size, uid_t target_uid)
{
	unsigned int i, j;
	size_t nlen, nlen16;
	char fake[BINDER_PKG_NAME_LEN];
	char needle16[BINDER_PKG_NAME_LEN * 2];
	char fake16[BINDER_PKG_NAME_LEN * 2];
	bool modified = false;

	if (!binder_enabled || !data || !size)
		return 0;
	if (!binder_hide_pkg_count)
		return 0;
	...
	for (i = 0; i < binder_hide_pkg_count; i++) {
		...
		/* UTF-16LE 模板（Android Parcel 的 String16 是 UTF-16LE） */
		nlen16 = nlen * 2;
		for (j = 0; j < nlen; j++) {
			needle16[j * 2] = needle[j];
			needle16[j * 2 + 1] = '\0';
			fake16[j * 2] = (needle[j] == '.') ? '.' : 'z';
			fake16[j * 2 + 1] = '\0';
		}
		...
		/* 模式 2：UTF-16LE 字节序列 */
		if (nlen16 <= size) {
			end = data + size - nlen16;
			p = data;
			while (p <= end) {
				p = memchr(p, needle16[0], end - p + 1);
				if (!p)
					break;
				if (memcmp(p, needle16, nlen16) == 0) {
					memcpy(p, fake16, nlen16);
					modified = true;
					p += nlen16;
				} else {
					p++;
...
EXPORT_SYMBOL_GPL(pmk_filter_binder);
//（注：内容由AI生成）
